...
# This is the tool called by the Main Chatbot to retrieve data from
# the product manuals. This tool consists of an agentic workflow.
def research_docs(user_input):
    # This function's goal is to return this dictionary containing 
    # chunks from the GROSS manuals. The dictionary starts out empty,
    # but the Research Agent will fill it with relevant chunks:
    rag_chunks = {}

    # The first step of the agentic workflow simultaneously
    # identifies the correct GROSS product AND expands the query:
    ...
    for _ in range(5): 
        ...
        for tool_call in tool_calls:
            ...
            if function_name == "search_manual":
                # The search_manual function searches the vector DB
                # and fills rag_chunks with relevant manual chunks:
                result = {"search_manual": search_manual(**args, \
                    manual_name=manual_name, rag_chunks=rag_chunks)}
            elif function_name == "read_webpage":
                # The read_webpage function looks up relevant web URLs
                # and fills rag_chunks with relevant info from the web:
...
# The Main Chatbot's system prompt:
